...
//   Firewall de aplicação web. Fica entre o usuário e o servidor, filtrando
//   requisições maliciosas (SQL injection, XSS, etc.). Exemplos: Cloudflare
...
//   Rede de distribuição de conteúdo. Servidores espalhados pelo mundo que
//   armazenam copias do site para entregar mais rapido ao usuário. Exemplos:
...
//   Sistema que detecta e bloqueia ataques de negacao de serviço distribuido.
//   Quando muitas requisições chegam de uma vez, o sistema pode ativar modos
//   de proteção como "Under Attack Mode" (Cloudflare) ou simplesmente
//   descartar conexões. Status 503 e comum quando isso acontece.
//
// Rate Limiting (Limitacao de Taxa):
//   Mecanismo que limita quantas requisições um cliente pode fazer em um
//   período de tempo. Exemplo: "100 requests por minuto". Quando o limite
...
//   Usam cookies especiais, fingerprinting de navegador e análise
...
/**
 * Categoria do mecanismo de proteção detectado.
 *
 * - waf:             Firewall de aplicação web (bloqueia requests maliciosos)
 * - cdn:             Rede de distribuição de conteúdo (cache e aceleracao)
 * - rate-limiter:    Limitador de taxa de requisições
 * - anti-bot:        Sistema de detecção e bloqueio de bots
 * - ddos-protection: Proteção contra ataques de negacao de serviço
 * - captcha:         Desafio interativo (CAPTCHA/challenge page)
 * - unknown:         Proteção identificada mas sem categoria definida
 */
type ProtectionType =
  | "waf"
  | "cdn"
...
/**
 * Provedor/fabricante do serviço de proteção detectado.
 */
...
/** Nivel qualitativo de confianca da detecção. */
type ConfidenceLevel = "high" | "medium" | "low";

/** Uma evidencia individual que levou a detecção de uma proteção. */
...
/** Resultado consolidado da detecção de um tipo de proteção de um provedor. */
...
/** Informações sobre rate limiting detectado durante o teste. */
...
/** Padrão comportamental observado na timeline do teste. */
...
/** Relatório completo gerado após a análise de protecoes. */
...
// ---------------------------------------------------------------------------
//
// Cada entrada mapeia um header HTTP para um provedor/tipo de proteção.
// O campo `pattern` pode ser uma RegExp ou string para matching.
// O campo `confidence` vai de 0 a 100 indicando a certeza da detecção.
// ---------------------------------------------------------------------------
...
const HEADER_SIGNATURES: HeaderSignature[] = [
  ...
  // O "cf-ray" e um identificador único por request, praticamente confirma Cloudflare.
  {
    header: "cf-ray",
    pattern: /.+/,
    provider: "cloudflare",
    type: "cdn",
    confidence: 95,
    detail: "Cloudflare Ray ID detectado",
  },
  {
  ...
  },
  {
    ...
    confidence: 95,
    detail: "Mitigacao Cloudflare ativa",
  },
  {
    header: "server",
    ...
    type: "waf",
    confidence: 90,
    detail: "Servidor Cloudflare identificado",
  },
  {
    ...
    type: "cdn",
    confidence: 80,
    detail: "Proxy Cloudflare detectado",
  },
  ...
  // Headers "x-akamai-*" identificam requests processados pela rede Akamai.
  {
    ...
    type: "cdn",
    confidence: 95,
    detail: "Transformacao Akamai detectada",
  },
  {
  ...
  },
  {
    ...
    type: "cdn",
    confidence: 90,
    detail: "Akamai GRN detectado",
  },
  {
    header: "server",
    pattern: /akamaighost|akamainetwork/i,
    provider: "akamai",
    type: "cdn",
    confidence: 90,
    detail: "Servidor Akamai identificado",
  },
  {
    ...
    type: "anti-bot",
    confidence: 85,
    detail: "Sessão anti-bot Akamai",
  },

  // --- Fastly ---
  // CDN focada em performance. Usa Varnish por baixo, então o header "via"
  // com "varnish" pode indicar Fastly (mas também Varnish standalone).
  {
    header: "x-served-by",
    pattern: /cache-/i,
    provider: "fastly",
    type: "cdn",
    confidence: 85,
    detail: "Fastly cache edge detectado",
  },
  {
  ...
  // WAF empresarial. O header "x-iinfo" e tipico do Incapsula (agora Imperva).
  {
    ...
    type: "waf",
    confidence: 90,
    detail: "Imperva Incapsula detectado",
  },
  {
    header: "x-cdn",
    pattern: /incapsula|imperva/i,
    provider: "imperva",
    type: "waf",
    confidence: 95,
    detail: "Imperva CDN header",
  },
  ...
  },
  {
    ...
    type: "waf",
    confidence: 90,
    detail: "Servidor Sucuri identificado",
  },
  {
  ...
  // Headers "x-amz-*" indicam infraestrutura AWS.
  {
    ...
    type: "waf",
    confidence: 70,
    detail: "AWS request ID (possível WAF)",
  },
  {
    header: "x-amz-cf-id",
    pattern: /.+/,
    ...
    type: "cdn",
    confidence: 90,
    detail: "CloudFront distribution ID",
  },
  {
    header: "x-amz-cf-pop",
    pattern: /.+/,
    ...
    type: "cdn",
    confidence: 90,
    detail: "CloudFront POP edge",
  },
  {
    header: "server",
    pattern: /amazons3|cloudfront/i,
    provider: "aws-cloudfront",
    type: "cdn",
    confidence: 85,
    detail: "AWS CloudFront detectado",
  },
  ...
  // CDN e balanceador de carga da Microsoft Azure.
  {
    ...
    type: "cdn",
    confidence: 90,
    detail: "Azure Front Door detectado",
  },
  {
    header: "x-fd-healthprobe",
    pattern: /.+/,
    provider: "azure-frontdoor",
    type: "cdn",
    confidence: 85,
    detail: "Azure Front Door health",
  },

  // --- Google Cloud Armor ---
  // WAF e proteção DDoS do Google Cloud Platform.
  {
    ...
    type: "waf",
    confidence: 70,
    detail: "Google Cloud request params",
  },
  {
    header: "server",
    pattern: /gws|google/i,
    provider: "google-cloud-armor",
    type: "cdn",
    confidence: 60,
    detail: "Servidor Google detectado",
  },

  // --- DDoS-Guard ---
  // Serviço russo de proteção DDoS. Identificavel pelo header "server".
  {
    header: "server",
    pattern: /ddos-guard/i,
    provider: "ddos-guard",
    type: "ddos-protection",
    confidence: 95,
    detail: "DDoS-Guard server detectado",
  },

  // --- StackPath ---
  // CDN com proteção WAF integrada.
  {
    header: "x-sp-url",
    pattern: /.+/,
    provider: "stackpath",
    type: "cdn",
    confidence: 85,
    detail: "StackPath CDN detectado",
  },
  ...
  // indica um proxy/cache na frente do servidor de origem.
  {
    ...
    type: "cdn",
    confidence: 80,
    detail: "Varnish cache detectado",
  },
  {
    header: "via",
    pattern: /varnish/i,
    ...
    type: "cdn",
    confidence: 75,
    detail: "Varnish proxy via header",
  },

  // --- Nginx ---
  // Servidor web muito usado como reverse proxy. Pode ter módulo de rate
  // limiting (ngx_http_limit_req_module), mas sozinho não confirma proteção.
  {
    header: "server",
    pattern: /^nginx/i,
    provider: "nginx",
    type: "waf",
    confidence: 40,
    detail: "Nginx server (possível rate limiter)",
  },
  ...
  // "retry-after":           segundos ate poder tentar novamente (RFC 7231)
  {
    ...
    type: "rate-limiter",
    confidence: 95,
    detail: "Rate limit header presente",
  },
  {
  ...
  },
  {
    ...
    type: "rate-limiter",
    confidence: 85,
    detail: "Retry-After header presente",
  },
  {
    ...
    type: "rate-limiter",
    confidence: 90,
    detail: "Rate limit (formato alternativo)",
  },
];
...
// ---------------------------------------------------------------------------
//
// Muitos serviços de proteção plantam cookies específicos no navegador para
// rastrear sessões, gerenciar challenges e identificar bots. O nome do cookie
// geralmente segue um padrão único do provedor.
// ---------------------------------------------------------------------------
...
const COOKIE_SIGNATURES: CookieSignature[] = [
  ...
  // "cf_clearance": plantado após o usuário passar por um challenge (CAPTCHA)
  {
    pattern: /^__cf_bm=/,
    provider: "cloudflare",
    type: "anti-bot",
  ...
  },
  {
    ...
    type: "captcha",
    confidence: 85,
    detail: "Cloudflare clearance (pos-challenge)",
  },
  {
    pattern: /^__cfduid=/,
    provider: "cloudflare",
    type: "cdn",
    confidence: 80,
    detail: "Cloudflare identificador",
  },
  {
    pattern: /^cf_ob_info=/,
    provider: "cloudflare",
    type: "ddos-protection",
    confidence: 80,
    detail: "Cloudflare Always Online cookie",
  },
  {
    pattern: /^_cf_/,
    provider: "cloudflare",
    type: "waf",
    confidence: 75,
    detail: "Cookie Cloudflare genérico",
  },
  ...
  // "_abck": cookie anti-bot principal do Akamai Bot Manager
  {
    ...
    type: "anti-bot",
    confidence: 90,
    detail: "Akamai Bot Manager cookie",
  },
  {
    pattern: /^bm_sv=/,
    provider: "akamai",
    type: "anti-bot",
    confidence: 85,
    detail: "Akamai Bot Manager session",
  },
  {
    pattern: /^bm_mi=/,
    provider: "akamai",
  ...
  },
  {
    ...
    type: "anti-bot",
    confidence: 90,
    detail: "Akamai anti-bot cookie (_abck)",
  },

  // --- Imperva / Incapsula ---
  // "visid_incap_*": visitor ID único por site protegido
  // "reese84": cookie do Advanced Bot Protection (detecção avançada de bots)
  {
    pattern: /^visid_incap_/,
    provider: "imperva",
    type: "waf",
    confidence: 90,
    detail: "Imperva Incapsula visitor ID",
  },
  {
    pattern: /^incap_ses_/,
    provider: "imperva",
    type: "waf",
    confidence: 90,
    detail: "Imperva Incapsula session",
  },
  {
    pattern: /^reese84=/,
    provider: "imperva",
    type: "anti-bot",
    confidence: 85,
    detail: "Imperva Advanced Bot Protection",
  },

  // --- Sucuri ---
  {
    pattern: /^sucuri_cloudproxy_/,
    provider: "sucuri",
    type: "waf",
    confidence: 90,
    detail: "Sucuri cloud proxy cookie",
  },
  ...
  // --- Outros provedores ---
  {
    ...
    type: "anti-bot",
    confidence: 90,
    detail: "DataDome anti-bot cookie",
  },
  {
    pattern: /^__ddg/,
    provider: "ddos-guard",
    type: "ddos-protection",
    confidence: 85,
    detail: "DDoS-Guard cookie",
  },
];

// ---------------------------------------------------------------------------
// Base de assinaturas - Conteúdo do Body (HTML)
// ---------------------------------------------------------------------------
//
...
const BODY_SIGNATURES: BodySignature[] = [
  ...
  // "Attention Required" = página de bloqueio do WAF
  {
    pattern: /cf-browser-verification|cf-challenge|cf\.challenge/i,
    provider: "cloudflare",
    type: "captcha",
    confidence: 90,
    detail: "Cloudflare challenge page detectada",
  },
  {
    ...
    type: "waf",
    confidence: 85,
    detail: "Cloudflare WAF block page",
  },
  {
    pattern: /checking your browser.*cloudflare/i,
    provider: "cloudflare",
    type: "anti-bot",
    confidence: 90,
    detail: "Cloudflare Under Attack Mode",
  },
  {
    pattern: /ray id:/i,
    provider: "cloudflare",
    type: "waf",
    confidence: 70,
    detail: "Cloudflare Ray ID na página de erro",
  },

  // --- Imperva / Incapsula ---
  {
    pattern: /access denied.*incapsula/i,
    provider: "imperva",
    type: "waf",
    confidence: 90,
    detail: "Imperva block page",
  },
  {
    pattern: /incident id.*incapsula/i,
    provider: "imperva",
    type: "waf",
    confidence: 85,
    detail: "Imperva incident ID",
  },

  // --- Sucuri ---
  {
    pattern: /sucuri website firewall/i,
    provider: "sucuri",
    type: "waf",
    confidence: 95,
    detail: "Sucuri WAF block page",
  },
  {
    pattern: /access denied.*sucuri/i,
    provider: "sucuri",
    type: "waf",
    confidence: 90,
    detail: "Sucuri access denied",
  },

  // --- Akamai ---
  {
    pattern: /akamai.*access denied|ghost.*access denied/i,
    provider: "akamai",
    type: "waf",
    confidence: 80,
    detail: "Akamai block page",
  },
  ...
  // CAPTCHAs de terceiros (podem aparecer em qualquer provedor)
  {
    ...
    type: "captcha",
    confidence: 80,
    detail: "CAPTCHA detectado na resposta",
  },
  {
    pattern: /<title>.*403.*forbidden.*<\/title>/i,
    provider: "unknown",
    type: "waf",
    confidence: 60,
    detail: "Página 403 Forbidden (possível WAF)",
  },
  {
    ...
    type: "rate-limiter",
    confidence: 75,
    detail: "Mensagem de rate limit no body",
  },
];

// ---------------------------------------------------------------------------
// Mapeamentos de nomes legiveis para exibicao no relatório
// ---------------------------------------------------------------------------

/** Nomes de exibicao para cada provedor de proteção. */
...
/** Labels legiveis para cada tipo de proteção. */
...
/** Intervalo mínimo entre coletas de amostra (em milissegundos). */
const SAMPLE_INTERVAL_MS = 2000;

/** Número máximo de amostras armazenadas para análise. */
const MAX_SAMPLES = 50;

/** Confianca maxima permitida (nunca atingimos 100% para evitar falsos absolutos). */
const MAX_CONFIDENCE = 99;

/** Mínimo de pontos na timeline para análise comportamental. */
const MIN_TIMELINE_POINTS = 3;

/** Amostras iniciais coletadas sem restrição de intervalo. */
const INITIAL_UNRESTRICTED_SAMPLES = 2;

// ---------------------------------------------------------------------------
// Limiares para detecção comportamental
// ---------------------------------------------------------------------------

/** Fator mínimo de aumento de latência para considerar throttling (3x = 300%). */
...
// ---------------------------------------------------------------------------
// Limiares para cálculo de risco geral
// ---------------------------------------------------------------------------

/** Pesos de risco por tipo de proteção (multiplicam a confianca). */
...
/** Pontos de risco adicionados por padrão comportamental. */
const BEHAVIORAL_RISK_POINTS: Readonly<
  Record<BehavioralPattern["type"], number>
> = {
  blocking: 80,
  throttling: 40,
  challenge: 60,
...
/** Limiares para classificação de risco geral. */
 ...
 * mecanismos de proteção (WAF, CDN, rate limiting, anti-bot, etc.).
 *
 * Funcionamento:
 ...
 * 2. Após o teste, `analyze()` processa todas as amostras coletadas contra
 *    as bases de assinaturas e gera um relatório completo.
 *
...
export class ProtectionDetector {
  ...
  /**
   * Registra uma amostra de resposta HTTP para análise posterior.
   *
   * Chamado pelo stress engine para um subconjunto de requests (amostragem).
   * As primeiras amostras são coletadas imediatamente; após isso, respeita
   * o intervalo mínimo de SAMPLE_INTERVAL_MS para evitar excesso de dados.
   */
  collectSample(sample: ResponseSample): void {
    if (this.collectedSamples.length >= MAX_SAMPLES) return;

    const now = Date.now();
    const isAfterInitialPhase =
      this.collectedSampleCount > INITIAL_UNRESTRICTED_SAMPLES;
  ...
  /** Vincula a timeline do teste para análise de padrões comportamentais. */
  setTimeline(timeline: SecondMetrics[]): void {
    this.timelineReference = timeline;
  }

  /**
   * Gera o relatório final de protecoes detectadas.
   *
   * Deve ser chamado após o teste finalizar. Analisa todas as amostras
   * coletadas contra as bases de assinaturas (headers, cookies, body,
   * status codes) e também avalia padrões comportamentais na timeline.
   */
  analyze(): ProtectionReport {
    ...
    // Fase 2: Analisar padrões de rate limiting e comportamentais na timeline
    const rateLimitInfo = this.extractRateLimitInfo();
    const behavioralPatterns = this.detectBehavioralPatterns();

    // Fase 3: Converter padrões comportamentais anomalos em deteccoes
    ...
    const detections = Array.from(detectionMap.values()).sort(
   ...
   * Cada header e comparado com todos os padrões conhecidos.
   */
  private detectFromHeaders(
    ...
    for (const signature of HEADER_SIGNATURES) {
      ...
      this.upsertDetection(detectionMap, {
        ...
        indicator,
        description: this.buildProviderDescription(
          signature.provider,
          signature.type,
        ),
      });
   ...
   * Cookies são verificados pelo nome (prefixo antes do "=").
   */
  ...
  /**
   * Analisa o snippet do body HTML contra padrões conhecidos de páginas
   * de bloqueio, challenge e erro de WAFs e protecoes DDoS.
   */
  private detectFromBody(
    sample: ResponseSample,
    detectionMap: Map<string, ProtectionDetection>,
  ): void {
    ...
    for (const signature of BODY_SIGNATURES) {
      ...
      // Body usa chave separada para não colidir com deteccoes por header/cookie
      ...
      if (existing) {
        existing.confidence = Math.min(
          MAX_CONFIDENCE,
          Math.max(existing.confidence, signature.confidence),
        );
        existing.confidenceLevel = this.resolveConfidenceLevel(
          existing.confidence,
        );
        continue;
      }
   ...
   *
   * Códigos relevantes:
   * - 403 (Forbidden): possível bloqueio por WAF
   * - 429 (Too Many Requests): rate limiting confirmado
   * - 503 (Service Unavailable): possível proteção DDoS ou sobrecarga
   */
  private detectFromStatusCode(
    sample: ResponseSample,
    detectionMap: Map<string, ProtectionDetection>,
  ): void {
    ...
    const statusSignatures: Array<{
      ...
      },
      {
        ...
        detail:
          "Rate limiting ativo -- servidor rejeitando requests excedentes",
      ...
      },
      {
        ...
        detail:
          "Status 503 pode indicar proteção DDoS ativa ou servidor sobrecarregado",
        description:
          "Status 503 detectado -- possível proteção DDoS ou sobrecarga",
      },
    ];
  ...
  /**
   * Extrai informações de rate limiting a partir dos headers das amostras
   * e dos padrões de status code na timeline.
   *
   * Verifica:
   * - Headers padrão de rate limit (x-ratelimit-*, retry-after)
   * - Presenca de status 429 nas amostras
   * - Ponto na timeline onde os 429s comecaram
   * - Padrão de recuperação (se os erros diminuem após o pico)
   */
  private extractRateLimitInfo(): RateLimitInfo {
    ...
    for (const sample of this.collectedSamples) {
      const limitHeader =
        sample.headers["x-ratelimit-limit"] ||
        sample.headers["x-rate-limit-limit"];
      const resetHeader =
        sample.headers["x-ratelimit-reset"] || sample.headers["retry-after"];
      ...
      if (resetHeader) {
        ...
        if (!isNaN(resetValue) && resetValue > 0) {
          ...
          // Se e um número simples, tratar como segundos diretamente
    ...
    const hasStatus429 = this.collectedSamples.some(
      (s) => s.statusCode === 429,
    );
   ...
   * Retorna undefined se não houver 429s na timeline.
   */
  ...
  private detectRecoveryPattern(): string | undefined {
    ...
    const errorRates = timeline.map((s) =>
      s.requests > 0 ? s.errors / s.requests : 0,
    );
    ...
    const isRecovering =
      ratesAfterPeak[ratesAfterPeak.length - 1] < ratesAfterPeak[0];

    return isRecovering
      ? "Parcialmente recuperavel -- taxa de erro diminui após pico"
      : "Sem recuperação -- bloqueio persistente";
  }
  ...
  /**
   * Analisa a timeline do teste para detectar padrões comportamentais
   * que indicam protecoes ativas, mesmo sem assinaturas explicitas.
   *
   * Padrões detectados:
   * - Throttling: latência cresce progressivamente (servidor desacelerando)
   * - Blocking: taxa de erro salta abruptamente (bloqueio ativado)
   * - Challenge: grande fracao de respostas 403/503 (páginas de challenge)
  ...
  private detectBehavioralPatterns(): BehavioralPattern[] {
    ...
    if (timeline.length < MIN_TIMELINE_POINTS) {
      patterns.push({
        type: "normal",
        description: "Dados insuficientes para análise comportamental",
        evidence: `Timeline < ${MIN_TIMELINE_POINTS} segundos`,
      });
      return patterns;
    }

    // Verificar cada tipo de padrão anomalo
    ...
    // Se nenhum padrão anomalo foi encontrado, reportar como normal
    if (patterns.length === 0) {
      patterns.push({
        type: "normal",
        description: "Nenhum padrão comportamental anomalo detectado",
        evidence: "Latência e throughput estaveis",
      });
    }
  ...
  /**
   * Detecta throttling: o servidor aumenta a latência progressivamente
   * para desacelerar o cliente. Compara a latência media do primeiro
   * terco do teste com o último terco.
   */
  private detectThrottling(
    ...
    const avgLatencyFirstThird = this.calculateAverage(
      latencies.slice(0, firstThirdEnd),
    );
    const avgLatencyLastThird = this.calculateAverage(
      latencies.slice(lastThirdStart),
    );

    if (avgLatencyFirstThird <= 0) return null;
    if (avgLatencyLastThird <= avgLatencyFirstThird * THROTTLING_LATENCY_FACTOR)
      return null;

    const increaseFactor = (avgLatencyLastThird / avgLatencyFirstThird).toFixed(
      1,
    );

    return {
      type: "throttling",
      description: `Throttling detectado: latência aumentou ${increaseFactor}x durante o teste`,
      startSecond: firstThirdEnd,
      evidence: `Latência media: ${avgLatencyFirstThird.toFixed(0)}ms -> ${avgLatencyLastThird.toFixed(0)}ms`,
    };
  }

  /**
   * Detecta bloqueio abrupto: taxa de erro salta de <20% para >50%
   * em um único segundo, indicando que uma proteção foi ativada.
   */
  private detectBlocking(timeline: SecondMetrics[]): BehavioralPattern | null {
    const errorRates = timeline.map((s) => {
      if (s.requests === 0) return 0;

      // Contar erros HTTP relevantes (403, 429 e 5xx) alem dos erros de conexão
      const httpErrorCount = Object.entries(s.statusCodes)
        .filter(
          ([code]) => code === "403" || code === "429" || Number(code) >= 500,
        )
    ...
    for (let i = 1; i < errorRates.length; i++) {
      ...
      if (
        currentRate > BLOCKING_ERROR_THRESHOLD &&
        previousRate < BLOCKING_NORMAL_THRESHOLD
      ) {
  ...
  private detectChallenge(timeline: SecondMetrics[]): BehavioralPattern | null {
    ...
    return {
      type: "challenge",
      description: `Challenge pages detectadas: ${(challengeRatio * 100).toFixed(1)}% das respostas são 403/503`,
  ...
  private detectDegradation(
    ...
    const avgRpsFirstThird = this.calculateAverage(
      rpsValues.slice(0, firstThirdEnd),
    );
    const avgRpsLastThird = this.calculateAverage(
      rpsValues.slice(lastThirdStart),
    );

    if (avgRpsFirstThird <= 0) return null;
    if (avgRpsLastThird >= avgRpsFirstThird * DEGRADATION_RPS_FACTOR)
      return null;

    const dropPercentage = (
      (1 - avgRpsLastThird / avgRpsFirstThird) *
      100
    ).toFixed(0);
  ...
  /**
   * Converte padrões comportamentais anomalos (throttling, blocking)
   * em deteccoes formais no mapa, para que aparecam no relatório.
   */
  ...
  // -------------------------------------------------------------------------
  // Cálculo de risco e geração de resumo
  // -------------------------------------------------------------------------

  /**
   * Calcula o nivel de risco geral com base nas deteccoes encontradas,
   * informações de rate limiting e padrões comportamentais.
   *
  ...
  private calculateOverallRisk(
    ...
    const hasNormalBehavior = behavioralPatterns.every(
      (b) => b.type === "normal",
    );
    ...
    // Somar pontos de risco por cada proteção detectada
    ...
    // Somar pontos por padrões comportamentais anomalos
  ...
  /**
   * Gera um resumo legível em português com as protecoes detectadas,
   * provedores identificados, rate limiting e padrões comportamentais.
   */
  private generateSummary(
    ...
    if (detections.length === 0 && !rateLimitInfo.detected) {
      return "Nenhuma proteção de seguranca visível detectada. O alvo não apresentou sinais de WAF, CDN ou rate limiting nas amostras analisadas.";
    }

    const summaryParts: string[] = [];

    // Listar provedores únicos detectados (ignorando "unknown")
    ...
    if (uniqueProviders.length > 0) {
      const providerNames = uniqueProviders.map(
        (p) => PROVIDER_DISPLAY_NAMES[p] ?? p,
      );
      summaryParts.push(`Provedores detectados: ${providerNames.join(", ")}`);
    }

    // Listar tipos de proteção únicos detectados
    const uniqueTypes = [...new Set(detections.map((d) => d.type))];
    const typeNames = uniqueTypes.map(
      (t) => PROTECTION_TYPE_SHORT_LABELS[t] ?? t,
    );
    summaryParts.push(`Tipos de proteção: ${typeNames.join(", ")}`);

    // Detalhes de rate limiting
    if (rateLimitInfo.detected) {
      const details: string[] = [];
      if (rateLimitInfo.limitPerWindow)
        details.push(`limite: ${rateLimitInfo.limitPerWindow}`);
      if (rateLimitInfo.triggerPoint)
        details.push(`ativado no segundo ${rateLimitInfo.triggerPoint}`);
    ...
    const anomalousPatterns = behavioralPatterns.filter(
      (b) => b.type !== "normal",
    );
    if (anomalousPatterns.length > 0) {
      const descriptions = anomalousPatterns
        .map((b) => b.description)
        .join("; ");
      summaryParts.push(`Padrões comportamentais: ${descriptions}`);
    }
  ...
  private upsertDetection(
    ...
    if (existing) {
      ...
      existing.confidence = Math.min(
        MAX_CONFIDENCE,
        Math.max(existing.confidence, params.confidence),
      );
      existing.confidenceLevel = this.resolveConfidenceLevel(
        existing.confidence,
      );

      // Adicionar indicador apenas se ainda não existe um com o mesmo nome
      const isDuplicate = existing.indicators.some(
        (i) => i.name === params.indicator.name,
      );
      if (!isDuplicate) {
  ...
  /** Gera descrição legível combinando nome do provedor e tipo de proteção. */
  private buildProviderDescription(
    provider: ProtectionProvider,
    type: ProtectionType,
  ): string {
  ...
  /** Calcula a media aritmetica de um array de números. Retorna 0 se vazio. */
