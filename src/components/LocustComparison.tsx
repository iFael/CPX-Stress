...
import { useLocust } from "@/hooks/useLocust";
import { formatMs } from "@/shared/test-analysis";
import type { LocustConfig } from "@/types";
...
interface LocustComparisonProps {
...
export function LocustComparison({
  cpxResult,
  config,
}: LocustComparisonProps) {
  const [available, setAvailable] = useState<boolean | null>(null);
  const { status, summary, error, progress, run } = useLocust();
  ...
  const rows = useMemo<ComparisonRow[]>(() => {
    const locustErrorRate = summary ? summary.errorRate * 100 : null;

    return [
      {
        label: "Avg latência",
        cpx: cpxResult.avgLatency,
        locust: summary?.avgLatency ?? null,
        unit: "ms",
      ...
      },
      {
        ...
        locust: summary?.p90Latency ?? null,
        unit: "ms",
      ...
      },
      {
        ...
        locust: summary?.p95Latency ?? null,
        unit: "ms",
      ...
      },
      {
        ...
        locust: summary?.p99Latency ?? null,
        unit: "ms",
      ...
      },
      {
        ...
        locust: locustErrorRate,
        unit: "%",
        delta: buildDeltaLabel(cpxResult.errorRate, locustErrorRate, "%"),
        tone:
          locustErrorRate !== null
            ? compareAbsoluteDelta(cpxResult.errorRate, locustErrorRate, 2, 5)
            : "neutral",
      },
    ];
  }, [cpxResult, summary]);
    ...
    <div className="bg-sf-surface border border-sf-border rounded-xl p-4 space-y-4">
      <div className="flex items-center justify-between gap-4">
        <div className="space-y-1">
          <h3 className="text-sm font-semibold text-sf-text flex items-center gap-2">
            <Scale className="w-4 h-4 text-sf-accent" />
            Comparação com Locust
          </h3>
          <p className="text-xs text-sf-textMuted">
            Executa o mesmo fluxo/config do teste com Locust para adicionar uma
            terceira referência independente de medição.
          </p>
        </div>
        ...
        {status !== "running" ? (
          <button
            ...
            className="text-xs px-3 py-2 rounded-lg bg-sf-accent hover:opacity-90 text-slate-950 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            <RefreshCw className="w-3.5 h-3.5" />
            {status === "done" ? "Rodar novamente" : "Comparar com Locust"}
          </button>
        ) : (
          <span className="text-xs text-sf-accent flex items-center gap-2">
      ...
      {available === false && (
        <div className="text-xs text-sf-warning bg-sf-warning/10 border border-sf-warning/30 rounded-lg p-3 flex items-start gap-2">
          ...
          <span>
            O binário do Locust não está disponível nesta máquina. Instale o
            Locust ou configure `LOCUST_PATH` para habilitar a comparação.
          </span>
        </div>
      )}
      ...
      {summary && (
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="px-2.5 py-0.5 rounded-full bg-sf-accent/20 text-sf-accent border border-sf-accent/30">
              {summary.version}
            </span>
            {summary.totalReqs > 0 && (
          ...
          <table className="w-full text-xs text-sf-textSecondary">
            <thead>
              <tr className="text-sf-textMuted border-b border-sf-border">
                ...
                <th className="text-right py-1.5">Locust</th>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.label} className="border-b border-sf-border/50">
                  ...
                  <td className="py-2 text-right font-mono">
                    {row.locust !== null ? formatValue(row.locust, row.unit) : "—"}
                  </td>
