 ...
 * HistoryPanel.tsx
 *
 * Painel de historico de testes -- exibe todos os testes ja realizados,
 * permitindo buscar, filtrar, ordenar e revisitar resultados anteriores.
 *
 * Funcionalidades:
 *  - Busca por URL ou metodo HTTP
 *  - Filtro por status (concluido, cancelado, erro)
 *  - Ordenacao por data, RPS, taxa de erros ou nota de saude
 *  - Agrupamento visual por periodo (hoje, ontem, esta semana, anteriores)
 *  - Exclusao individual com confirmacao e limpeza total do historico
...
type SortOption = 'date-desc' | 'date-asc' | 'rps-desc' | 'errors-desc' | 'health-desc'

/** Filtro por status do teste */
type StatusFilter = 'all' | 'completed' | 'cancelled' | 'error'

/** Rotulos legíveis para cada opcao de ordenacao */
...
/** Rotulos legiveis para cada filtro de status */
...
/* =========================================================================
   Funcao utilitaria -- calculo rapido de "nota de saude" do teste
   (usada para o indicador colorido e para ordenacao)
   ========================================================================= */

function getQuickScore(result: TestResult): number {
  // Falha total: todas as requisicoes falharam ou taxa de erro >= 95%
  if (
    result.errorRate >= 95 ||
  ...
  // Penalidade se a latencia P95 for alta (site demorou pra responder)
  if (result.latency.p95 > 5000) score -= 20
  else if (result.latency.p95 > 2000) score -= 10

  // Penalidade quando nenhum byte foi recebido (possivel bloqueio total)
...
/**
 * Retorna um rotulo acessivel descrevendo a nota de saude para leitores de tela.
 */
...
/* =========================================================================
   Funcao utilitaria -- agrupamento por periodo
   ========================================================================= */
...
/**
 * Agrupa os testes por periodo (Hoje, Ontem, Esta semana, Anteriores)
 * para facilitar a navegacao visual na lista.
 */
function groupByDate(results: TestResult[]): DateGroup[] {
  ...
  if (groups.today.length > 0) output.push({ label: 'Hoje', items: groups.today })
  if (groups.yesterday.length > 0) output.push({ label: 'Ontem', items: groups.yesterday })
  if (groups.thisWeek.length > 0) output.push({ label: 'Esta semana', items: groups.thisWeek })
  if (groups.older.length > 0) output.push({ label: 'Anteriores', items: groups.older })
  return output
}
...
export function HistoryPanel() {
  ...
  /* Referencia para fechar menus ao clicar fora */
  ...
  // causando um vazamento de memoria (a closure mantem referencias vivas).
  ...
  const processedResults = useMemo(() => {
    ...
    // 1. Filtrar por texto de busca (URL ou metodo HTTP)
  ...
  /* -----------------------------------------------------------------------
     Handlers -- acoes do usuario
     ----------------------------------------------------------------------- */

  /** Abre os detalhes de um teste especifico */
  ...
  /** Exclui um teste individual (com confirmacao inline) */
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      // Segundo clique: efetiva a exclusao
      setBusyDeleteId(id)
      try {
  ...
  /** Limpa todo o historico (com confirmacao em dois cliques) */
  const handleClearAll = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[StressFlow] Falha ao limpar historico:', err)
    }
  ...
  return (
    <div className="animate-slide-up" role="region" aria-label="Histórico de testes">

      {/* ---- Cabecalho: titulo + botao de limpar tudo ---- */}
      <header className="flex items-center justify-between mb-6">
        ...
        {history.length > 0 && (
          <button
            ...
            aria-label={confirmClear ? 'Confirmar limpeza do hist\u00F3rico' : 'Limpar todo o hist\u00F3rico'}
          ...
          >
            ...
            {clearingAll ? 'Limpando...' : confirmClear ? 'Confirmar Limpeza' : 'Limpar Tudo'}
          </button>
        )}
      </header>
      ...
      {history.length > 0 && (
        <div className="space-y-3 mb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Botao para limpar filtros quando ha filtros ativos */}
            {hasActiveFilters && (
              <button
                type="button"
            ...
            {hasActiveFilters && (
              <span
                ...
                aria-live="polite"
              >
                {processedResults.length} de {history.length} teste{history.length !== 1 ? 's' : ''}
              </span>
            )}
          </div>
        </div>
      ...
      {/* ---- Estado vazio: nenhum teste no historico ---- */}
      {history.length === 0 && (
        <EmptyState
          icon={<Calendar className="w-12 h-12 text-sf-textMuted" aria-hidden="true" />}
      ...
      {processedResults.length > 0 && (
        <div aria-label="Lista de testes realizados">
          {dateGroups
            ? /* Exibicao agrupada por periodo (quando ordenado por data) */
              dateGroups.map((group) => (
                <div key={group.label} className="mb-4">
                  <h3 className="text-xs font-semibold text-sf-textMuted uppercase tracking-wider mb-2 px-1">
                    {group.label}
                  </h3>
                  <div className="space-y-2" role="list" aria-label={`Testes: ${group.label}`}>
            ...
            : /* Exibicao linear (quando ordenado por metrica) */
              <div className="space-y-2" role="list" aria-label="Lista de testes realizados">
      ...
      {processedResults.length === 0 && history.length > 0 && (
        <EmptyState
          icon={<Search className="w-10 h-10 text-sf-textMuted" aria-hidden="true" />}
...
/* =========================================================================
   Subcomponente -- Item individual do historico
...
// Em historicos com dezenas de testes, cada interacao (busca, filtro, confirmacao de
// exclusao) causaria re-render de TODOS os itens. Com memo, apenas itens cujas props
// realmente mudaram (ex: isConfirmingDelete) sao re-renderizados.
// Para listas muito grandes (100+ itens), considerar virtualizacao com react-window.
const HistoryItem = memo(function HistoryItem({ result, isConfirmingDelete, isBusyDeleting, onView, onDelete }: HistoryItemProps) {
  const healthScore = getQuickScore(result)

  /** Rotulo descritivo do status para leitores de tela */
  const statusLabel =
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* URL do teste */}
            <span className="font-medium text-sf-text truncate" title={result.url}>
              {result.url}
            </span>
            ...
            {/* Badge: status se nao foi concluido normalmente */}
            {result.status === 'cancelled' && (
          ...
          {/* Linha de metadados: data, duracao, RPS, usuarios, erros */}
          <div className="flex items-center gap-4 mt-2 text-xs text-sf-textMuted flex-wrap">
            ...
            <span className="flex items-center gap-1" title="Requisições por segundo">
              <Activity className="w-3 h-3" aria-hidden="true" />
              {result.rps} RPS
            </span>

            {/* Numero de usuarios virtuais */}
            <span className="flex items-center gap-1" title="Usuários virtuais simultâneos">
        ...
        {/* ---- Botoes de acao (ver detalhes / excluir) ---- */}
        <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
          {/* Botao: ver detalhes do teste */}
          <button
            type="button"
          ...
          {/* Botao: excluir teste (com confirmacao inline) */}
          <button
            type="button"
...
/* =========================================================================
   ...
   Exibido quando nao ha testes ou quando a busca nao retorna resultados.
   ========================================================================= */
...
function EmptyState({ icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="text-center py-16" role="status">
      ...
      <p className="text-sm text-sf-textSecondary max-w-sm mx-auto">{description}</p>
      {action}
    </div>
  )
}
