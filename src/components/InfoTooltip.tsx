...
//
// Exibe um pequeno icone de "informação" (i). Quando o usuário
...
interface TooltipPosition {
  ...
  vertical: "top" | "bottom";
  /** Deslocamento horizontal em pixels (para não vazar da tela) */
...
export function InfoTooltip({ text, className = "" }: InfoTooltipProps) {
  ...
  // Referência ao balao do tooltip (para medir tamanho e posição)
  const tooltipRef = useRef<HTMLDivElement>(null);

  // Referência ao botão que aciona o tooltip
  const triggerRef = useRef<HTMLButtonElement>(null);

  // ID único para conectar o botão ao tooltip (acessibilidade)
  const tooltipId = useId();

  // ── Calcular posição ideal do tooltip ──────────────────────
  // Verifica se o tooltip cabe acima do icone. Se não couber,
  // posiciona abaixo. Também ajusta o eixo horizontal para que
  // o conteúdo não saia da area visível da janela.
  const updatePosition = useCallback(() => {
    ...
    // Margem de seguranca para não encostar na borda da janela
    ...
    // Se vazar pela direita, empurra para a esquerda
      ...
      tooltipCenter + halfTooltip >
      window.innerWidth - VIEWPORT_PADDING
    ) {
      horizontalOffset =
        window.innerWidth - VIEWPORT_PADDING - (tooltipCenter + halfTooltip);
    }
  ...
  // Isso permite que o usuário feche o tooltip clicando em
  ...
  // Permite que usuários de teclado fechem o tooltip facilmente.
  useEffect(() => {
    if (!visible) return;
    ...
    const handleEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        setVisible(false);
        // Devolve o foco para o botão ao fechar
  ...
  /** Mostra o tooltip quando o botão recebe foco via teclado */
  const handleFocus = useCallback(() => {
    setVisible(true);
  }, []);

  /** Esconde o tooltip quando o botão perde o foco */
  const handleBlur = useCallback(() => {
    setVisible(false);
  }, []);

  // ── Classes CSS do tooltip ─────────────────────────────────
  // Monta as classes de posição de acordo com o cálculo feito.

  // Posição vertical: acima ou abaixo do icone
  const verticalClasses =
    position.vertical === "top"
      ? "bottom-full mb-2" // Acima do icone, com espaço
      : "top-full mt-2"; // Abaixo do icone, com espaço
    ...
    <span className={`relative inline-flex items-center ${className}`}>
      ...
      <button
        ...
        className={[
          ...
          // Cursor de ajuda indica que ha informação extra
          "cursor-help",
        ].join(" ")}
      ...
      {visible && (
        <div
          ...
          id={tooltipId}
          role="tooltip"
          // Estilo inline necessário: o deslocamento horizontal e calculado
          // dinamicamente em tempo de execução com base na posição do tooltip
          // na viewport, portanto não pode ser expresso como classe Tailwind.
          style={{
            transform: `translateX(calc(-50% + ${position.horizontalOffset}px))`,
          }}
          className={[
            // Posição flutuante sobre o conteúdo
            ...
            // Texto legível e bem espacado
            "text-xs leading-relaxed text-sf-text",
            // Animação de entrada: surge suavemente
